      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-6 flex justify-center">
          <div className="inline-flex rounded-lg border border-gray-300 p-0.5">
            <button onClick={() => setMode("upload")} className={`rounded-md px-4 py-2 text-sm font-medium transition-all ${mode === "upload" ? "bg-rose-600 text-white shadow-sm" : "text-gray-600 hover:text-gray-900"}`}>
              Upload PDF
            </button>
            <button onClick={() => setMode("url")} className={`rounded-md px-4 py-2 text-sm font-medium transition-all ${mode === "url" ? "bg-rose-600 text-white shadow-sm" : "text-gray-600 hover:text-gray-900"}`}>
              Paste URL
            </button>
          </div>
        </div>
        ...
        {mode === "upload" ? (
          ...
          <div className="flex gap-3">
            <input
              ...
              className="flex-1 rounded-lg border border-gray-300 px-4 py-3 text-gray-900 shadow-sm transition-colors focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20"
            />
            <button
              onClick={extractFromUrl}
              disabled={loading}
              className="rounded-lg bg-rose-600 px-6 py-3 font-semibold text-white shadow-sm transition-all hover:bg-rose-700 disabled:opacity-50"
            >
              Extract
            </button>
          </div>
        ...
        {loading && (
          <div className="mt-8 flex flex-col items-center gap-3 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 p-8">
            <svg className="h-10 w-10 animate-spin text-rose-500" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
        ...
        {result && (
          <div className="mt-8 space-y-4 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="flex flex-wrap gap-4 text-sm">
              ...
              <div><span className="text-gray-500">Pages:</span> <span className="font-semibold text-rose-600">{result.pages}</span></div>
              <div><span className="text-gray-500">Words:</span> <span className="font-semibold text-rose-600">{wordCount.toLocaleString()}</span></div>
              {result.fileSize && (
